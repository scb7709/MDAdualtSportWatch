...
public class FormatCurrentData {
    ...
    public static String getTimeRange(String mTime) {
        ...
        int YEAR_interval = startTime.getYear() - curDate.getYear();
        int NONTH_interval = startTime.getMonth() - curDate.getMonth();
        int DAY_interval = startTime.getDay() - startTime.getDay();

        if (YEAR_interval == 0 && NONTH_interval == 0) {
            if (DAY_interval == 1) {
                return "昨天";
            } else if (DAY_interval < 15&&DAY_interval>1) {
                return DAY_interval + "天前";
            }
        }
        ...
        if (elapsedTime < seconds_of_15days) {
            if (elapsedTime / seconds_of_1day < 1) {
                return "昨天";
            }
